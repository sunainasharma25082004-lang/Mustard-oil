...
const calculateShippingWeight = (items) => {
  if (!Array.isArray(items) || items.length === 0) return 0.5;

  const totalWeight = items.reduce((sum, item) => {
    const quantity = Math.max(1, Number(item.quantity) || 1);
    const weightPerUnit = Number(item.shippingWeightKg) || 0.5;
...
const resolveDeliveryCharge = async ({ pincode, items, totalQuantity }) => {
  ...
  const weight =
    calculateShippingWeight(items) ||
    Math.max(
      0.1,
      Number((Number(config.defaultWeight) || 0.5) * units).toFixed(2),
    );
...
const buildOrderFromItems = async (items, customer) => {
  ...
  for (const item of items) {
    ...
    if (!product || !product.isActive || !product.inStock) {
      throw new Error(
        `Product unavailable: ${item.productName || item.productId}`,
      );
    }
    ...
    orderItems.push({
      ...
      quantity,
      price: product.price,
      shippingWeightKg: Number(product.shippingWeightKg) || 0.5,
      shippingLengthCm: Number(product.shippingLengthCm) || 20,
      shippingBreadthCm: Number(product.shippingBreadthCm) || 15,
      shippingHeightCm: Number(product.shippingHeightCm) || 10,
    });
  }
