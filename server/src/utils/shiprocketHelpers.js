...
const getDecryptedCredentials = (settings) => {
  ...
  try {
    ...
    if (!password) {
      throw new Error('Saved password could not be decrypted — re-enter and save credentials');
    }
    return { email, password };
  } catch (err) {
    throw new Error(err.message || 'Re-enter Shiprocket password and save again in admin');
  }
};
...
const authenticateShiprocket = async (email, password) => {
  ...
  try {
    ...
    throw new Error(`Cannot reach Shiprocket API — check internet connection (${err.message})`);
  }
  ...
  if (!response.ok || !data.token) {
    ...
    const hint =
      response.status === 403 || /invalid email and password/i.test(apiMessage || '')
...
const getShiprocketToken = async () => {
  ...
  if (settings.token && settings.tokenExpiresAt && settings.tokenExpiresAt > new Date()) {
    try {
...
const shiprocketRequest = async (method, path, body) => {
  ...
  if (data && typeof data === 'object' && Number(data.awb_assign_status) === 0) {
    throw new Error(formatShiprocketError(data, response.status));
  }
...
const extractFromOrderRecord = (record) => {
  ...
  const shiprocketOrderId = record.id ?? record.order_id ?? record.sr_order_id ?? null;
  ...
  return {
    ...
    channelOrderId: channelOrderId ? normalizeChannelOrderId(channelOrderId) : null,
  };
};
...
const extractFromCreateResponse = (result) => {
  ...
  const blocks = [result, result.payload, result.data, result.response?.data].filter(
    (b) => b && typeof b === 'object' && !Array.isArray(b)
  );

  for (const block of blocks) {
    const shipmentId = block.shipment_id ?? block.shipmentId ?? block.sr_shipment_id ?? null;
    const shiprocketOrderId = block.order_id ?? block.orderId ?? block.sr_order_id ?? null;
...
const fetchShiprocketOrderDetails = async (shiprocketOrderId) => {
  const data = await shiprocketRequest('GET', `/orders/show/${shiprocketOrderId}`);
...
const getShiprocketOrderByChannelId = async (channelOrderId) => {
  ...
  if (!orders.length) {
    ...
    orders = (searched?.data || []).filter((o) => orderMatchesChannelId(o, target));
  }
...
const resolveShiprocketIdsForOrder = async (order, createResult) => {
  if (
    ...
    return { ...createResult, channelOrderId: order.orderNumber, verified: true };
  }
  ...
  if (!ids.shipmentId && ids.shiprocketOrderId) {
    ...
    ids = { ...ids, ...details, channelOrderId: ids.channelOrderId || details.channelOrderId };
  }
  ...
  if (ids.channelOrderId && normalizeChannelOrderId(ids.channelOrderId) !== order.orderNumber) {
    throw new Error(
...
const getPickupLocations = async () => {
  ...
  const locations = data?.data?.shipping_address || data?.shipping_address || data?.data || data;
...
const calculateOrderWeight = (order, settings) => {
  const unitWeight = Number(settings.defaultWeight) || 0.5;
  const totalUnits = order.items.reduce((sum, item) => sum + item.quantity, 0);
  return Math.max(0.1, Number((unitWeight * totalUnits).toFixed(2)));
};
...
const buildShiprocketOrderPayload = (order, settings, pickupLocation) => {
  ...
  if (!phone || phone.length < 10) {
    throw new Error('Valid 10-digit customer phone is required for Shiprocket shipment');
  }
  ...
  const payload = {
    ...
    billing_email: order.customer.email || settings.companyEmail || 'karyorfarms@gmail.com',
    billing_phone: phone,
    ...
    length: settings.defaultLength || 20,
    breadth: settings.defaultBreadth || 15,
    height: settings.defaultHeight || 10,
    weight: calculateOrderWeight(order, settings),
  };
...
const checkServiceability = async ({ pickupPincode, deliveryPincode, weight, cod = false }) => {
  const settings = await getShiprocketSettings();
  const pickup = String(pickupPincode || settings.pickupPincode || settings.companyPincode).trim();
  const delivery = String(deliveryPincode).trim();

  if (!pickup || !delivery) {
    throw new Error('Pickup and delivery pincodes are required for serviceability check');
  }
  ...
  const data = await shiprocketRequest('GET', `/courier/serviceability/?${params.toString()}`);
...
const createShiprocketOrder = async (order) => {
  ...
  console.log('[Shiprocket] Creating order:', order.orderNumber, 'pickup:', pickupLocation);
  const result = await shiprocketRequest('POST', '/orders/create/adhoc', payload);
  console.log(
...
const trackByAwb = async (awb) => shiprocketRequest('GET', `/courier/track/awb/${awb}`);
...
const applyShipmentResultToOrder = (order, result, { error } = {}) => {
  ...
  if (ids.shiprocketOrderId) order.shiprocket.shiprocketOrderId = ids.shiprocketOrderId;
  if (ids.channelOrderId) order.shiprocket.channelOrderId = ids.channelOrderId;
  if (ids.verified) order.shiprocket.verified = true;

  const awbBlocks = [result, result?.response?.data, result?.payload, result?.data].filter(Boolean);
  let awb = null;
  ...
  try {
    ...
    if (settings.autoAssignAwb) {
      try {
        ...
        console.warn(`[Shiprocket] AWB assignment failed for ${order.orderNumber}:`, awbError.message);
      }
    }
    ...
    await order.save();
    console.error(`[Shiprocket] Shipment creation failed for ${order.orderNumber}:`, error.message);
...
const syncOrderFromWebhook = async (payload) => {
  ...
  const statusLabel = payload?.current_status || payload?.shipment_status || payload?.status;
  ...
  if (payload?.courier_name) order.shiprocket.courierName = payload.courier_name;
  if (statusLabel) {
...
const getPublicShippingConfig = async () => {
  const settings = await getShiprocketSettings();
  return {
    shiprocketEnabled: Boolean(settings.enabled && hasStoredCredentials(settings)),
    pickupPincode: settings.pickupPincode || settings.companyPincode || '',
    defaultWeight: settings.defaultWeight ?? 0.5,
  };
};
