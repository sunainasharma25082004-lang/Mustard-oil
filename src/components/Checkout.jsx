...
function Checkout() {
  ...
  useEffect(() => {
    settingsApi
      .getDelivery()
      .then((res) => setDeliveryDays(res.data.defaultDeliveryDays))
      .catch(() => {});
    paymentConfigApi
      .getConfig()
  ...
  useEffect(() => {
    ...
    if (!/^\d{6}$/.test(pincode) || items.length === 0) return;
  ...
  const handleGoogleSignInError = (message) => {
    ...
    setError(
      message?.includes("origin") || message?.toLowerCase().includes("blocked")
        ? `Google OAuth blocked (origin_mismatch). Google Cloud Console → Credentials → OAuth client → Authorized JavaScript origins mein ye URL add karo: ${origin}`
  ...
  if (success) {
    return (
      <>
        ...
        `}</style>
        <div
          className="checkout-page checkout-success-page"
          style={{
            minHeight: "100vh",
            background: "#0b0b0b",
            padding: "120px 20px",
          ...
          >
            ...
            {success.paymentMethod === "online" &&
              success.paymentStatus === "paid" && (
                <p style={{ color: "#4ade80", marginTop: 10 }}>
            ...
            {success.expectedDeliveryDate && (
              ...
              >
                ...
                <p style={{ color: "#aaa", margin: 0, fontSize: "0.88rem" }}>
                  Within <strong>{success.deliveryDays} business days</strong>{" "}
                  from today
                </p>
              </div>
            )}
            ...
            <div
              className="checkout-success-tracking"
        ...
        <div className="checkout-container">
          <div className="checkout-card">
            <div className="checkout-steps">
              <div
                className={`checkout-step ${step === 1 ? "active" : "done"}`}
              >
                1. Shipping
              </div>
            ...
            {step === 1 ? (
              <form onSubmit={goToPayment}>
                  ...
                  <div className="form-group">
                    ...
                    {pincodeStatus && (
                      ...
                      >
                        ...
                        pincodeStatus.deliveryCharge != null
                          ? ` · Delivery ₹${pincodeStatus.deliveryCharge}`
                          : ""}
                      </p>
                    )}
                  </div>
                ...
                >
                  No sign-in required for shipping details. You will be asked to
                  sign in securely before completing payment.
                </p>
                <button
                  type="submit"
              ...
              <>
                ...
                >
                  ...
                  <strong style={{ color: "#d4af37" }}>
                    {deliveryQuote?.estimatedDeliveryDays || deliveryDays} days
                  </strong>{" "}
                  after order confirmation
                  {deliveryQuote?.shiprocketEnabled && deliveryCharge > 0 && (
                    <>
                      {" "}
                ...
                {!user ? (
                  ...
                  >
                    ...
                    >
                      To place an order and track your purchase, please sign in
                    ...
                    >
                      You can complete payment process after Sign In.
                    </div>
                  </div>
                ) : (
                  <>
                    ...
                    <button
                      ...
                      disabled={
                        loading || items.length === 0 || !paymentConfig.enabled
                      }
                    >
                      {loading
                        ? "Processing..."
