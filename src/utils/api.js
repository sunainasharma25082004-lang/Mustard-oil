...
if (import.meta.env.DEV && typeof window !== "undefined") {
  console.log(
    "%c[Karyor API] Base URL:",
    "color:#666",
    API_URL || "(relative → Vite proxy)",
  );
}
...
export const shippingApi = {
  ...
  checkServiceability: (pincode, items) =>
    apiFetch("/api/shipping/serviceability", {
      method: "POST",
...
export const locationApi = {
  byPincode: (pincode) =>
    apiFetch(`/api/location/pincode/${encodeURIComponent(pincode)}`),
...
export const paymentApi = {
  createRazorpayOrder: (body) =>
    apiFetch("/api/payments/razorpay/create", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  verifyRazorpayPayment: (body) =>
    apiFetch("/api/payments/razorpay/verify", {
      method: "POST",
      body: JSON.stringify(body),
    }),
};
...
export const distributorApi = {
  apply: (body) =>
    apiFetch("/api/distributor", {
      method: "POST",
      body: JSON.stringify(body),
    }),
  getStatus: (phone) =>
    apiFetch(`/api/distributor/status/${encodeURIComponent(phone)}`),
};
